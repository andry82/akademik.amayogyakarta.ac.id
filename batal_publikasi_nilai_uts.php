...
while ($data = mysqli_fetch_array($res)) {
    $nim = $data['NIMHSTRNLM'];
    $nilai = $data['MID'];
    mysqli_query($mysqli, "UPDATE  trnlm SET NMID='0' WHERE NIMHSTRNLM='$nim' AND KDKMKTRNLM='$kdmk' AND THSMSTRNLM='$ta_lengkap'"); 
}
...
while ($datadp = mysqli_fetch_array($dp)) {
    mysqli_query($mysqli, "UPDATE dosen_pengajar SET PUBUTS='0' WHERE KDMK='$kdmk' AND THSMS='$ta_lengkap' AND KLSMHS like '$kelas%'");
    }
