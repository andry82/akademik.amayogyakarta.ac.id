...
while ($data = mysqli_fetch_array($data_kk)) {
    $nim = $data['NIMHSTRNLM'];
    $nilai = $data['MID'];
    mysqli_query($mysqli, "UPDATE  trnlm SET NMID='$nilai' WHERE NIMHSTRNLM='$nim' AND KDKMKTRNLM='$kodemk'");
}
mysqli_query($mysqli, "UPDATE dosen_kelompok SET PUBUTS='1' WHERE id='$id_kelompok'");
